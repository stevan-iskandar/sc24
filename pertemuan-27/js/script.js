...
function print() {
  ...
  while (result.length > 1) {
    ...
    result.forEach((calc, index) => {
      if ([
        ...
        const value1 = parseFloat(result[index - 1])
        const value2 = parseFloat(result[index + 1])
        const value = calc === buttonPlus.innerHTML
          ? value1 + value2
          : value1 - value2
        result.splice(index - 1, 3, value)
        return
  ...
  resultTag.innerHTML = ![undefined, null, NaN].includes(result[0]) ? result[0] : ''
}
